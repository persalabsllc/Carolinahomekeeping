...
test('reported six-hour quote reserves 3h35 without changing the price',()=>{
 ...
 const visit={id:'reported',kind:'booking' as const,...appointmentInterval(eastern('2026-09-21T08:00:00'),duration)};
 assert.equal(visit.ends_at,eastern('2026-09-21T11:35:00'));
 const slots=availableAppointments(120,[visit],1,now,0).filter(s=>localDay(s.starts_at)==='2026-09-21');
 assert.equal(slots[0].starts_at,eastern('2026-09-21T12:00:00'));
 assert.ok(!slots.some(s=>s.starts_at===eastern('2026-09-21T11:30:00')));
 assert.ok(validAppointment(appointmentInterval(eastern('2026-09-21T14:00:00'),duration),now,0));
 assert.equal(validAppointment(appointmentInterval(eastern('2026-09-21T14:30:00'),duration),now,0),false);
 ...
 assert.equal(hasSeriesCapacity({...series,id:'overlap',anchorStart:eastern('2026-09-28T11:30:00'),durationMinutes:120},snapshot,1),false);
 assert.equal(hasSeriesCapacity({...series,id:'after',anchorStart:eastern('2026-09-28T12:00:00'),durationMinutes:120},snapshot,1),true);
});
