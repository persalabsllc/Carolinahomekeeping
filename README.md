...
For the legacy header defect only, Control Room → Emails offers **Retry rejected email** on never-accepted, first-attempt failures with string-valued headers. Confirm that the exact Resend request returned HTTP 422 before using it. The authenticated, origin-checked action locks that row, checks current eligibility, repairs its headers, records the administrator and previous attempt time in `audit_log`, and processes only that message. It preserves the total attempt count and original idempotency key. Accepted, ambiguous, expired or ineligible messages cannot use this repair; the normal pending-email button still does not reopen failed mail.
...
- Owner-operated launch schedule (September 20, 2026): Thursday and Sunday only, 8 AM–5 PM in America/New_York. Other days are unavailable, never represented as fabricated bookings. Start options are generated every 30 minutes for the next 90 calendar days, honoring the configured minimum notice. Starts must be before 5 PM; full estimated duration may extend up to one hour after closing, through 6 PM on either operating day. No manual opening of individual windows is needed. The booking API, reservation validation, live homepage/ad-page highlights and Control Room calendar all use the same `workingHours` rule.
- The idempotent `applyOwnerLaunchSchedule` migration sets concurrent capacity to one cleaner, preserving pricing, durations and all existing reservations. It refuses a capacity reduction that conflicts with existing bookings/holds/recurring plans. A settings revision records the change, so later admin capacity adjustments are not reset on redeployment. Changing operating days only restricts new bookings/reschedules; any existing appointments or recurring commitments outside those days need owner review and are not silently canceled.
