...
export function checkoutParameters(hold:Record<string,any>,origin:string):Stripe.Checkout.SessionCreateParams{
 ...
 return {...common,mode:'subscription',payment_method_collection:'always',
  ...
  line_items:[{...line,price_data:{...line.price_data,product_data:{...product,name:product.name+' — first visit'}}},{...line,price_data:{...line.price_data,recurring:{interval:'week',interval_count:weeks}}}],
  ...
  custom_text:{submit:{message:`${money(quote.total)} for your first visit today, then ${money(quote.total)} every ${weeks===1?'week':weeks+' weeks'}, starting the day before your second visit. Selected extras repeat. Renews automatically until canceled. Manage or cancel online using your confirmation link.`}},
 };
}
