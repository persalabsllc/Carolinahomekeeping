 ...
 return sql.begin(async tx=>{
  ...
  if(input.frequency!=='once'){
   ...
   [plan]=await tx`insert into recurring_plans(customer_id,home_id,frequency,status,preferences,stripe_subscription_id,stripe_customer_id,initial_invoice_id,anchor_start,duration_minutes,interval_weeks,amount,quote,details)
    values(${customer.id},${home.id},${input.frequency},'trialing',${tx.json({service:input.service,addons:input.addons})},${subscriptionId},${customerId||null},${invoiceId||null},${slot.starts_at},${hold.quote.durationMinutes},${intervalWeeks(input.frequency)},${quote.total},${tx.json(hold.quote)},${tx.json(input)}) returning *`;
  ...
  const html=`<div style="font-family:Arial,sans-serif;color:#07364b;max-width:620px;margin:auto;padding:30px"><h1>Your home is handled.</h1><p>Hi ${escapeHtml(input.contact.name)}, your cleaning is confirmed.</p><p><strong>${reference}</strong></p><p>${escapeHtml(serviceNames[input.service])}<br>${date}<br>Scheduled start: ${time(slot.starts_at)} Eastern<br>Estimated finish: ${time(slot.ends_at)} Eastern</p><p>${escapeHtml(input.home.address)}, ${escapeHtml(input.home.city)}, NC ${input.zip}</p><p><strong>Paid: ${money(quote.total)}</strong></p>${quote.addons.length?'<p>Extras: '+quote.addons.map(a=>`${a.quantity} × ${escapeHtml(a.name)}`).join(', ')+'</p>':''}${input.frequency!=='once'?'<p>Your '+frequencyNames[input.frequency].toLowerCase()+' subscription is active. Your cleaning and selected extras repeat at the same weekday and Eastern time. '+money(quote.total)+' is billed automatically each cycle, starting the day before your second visit ('+new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',dateStyle:'medium'}).format(new Date(nextRenewal(input.scheduledStart,intervalWeeks(input.frequency))))+').</p><p><a href="'+manageUrl+'">Manage or cancel your subscription</a>. Canceling stops future renewals; prepaid appointments follow the cancellation policy.</p>':''}<p>Please secure pets and valuables, and make surfaces accessible. We have reserved this full time for your cleaning. The finish time is an estimate based on your selected services.</p><p>Need to change something? Reply to this email or <a href="${process.env.APP_URL}/contact">contact us</a>. Free cancellation more than 48 hours before arrival; 24–48 hours: 25% or a reschedule credit; less than 24 hours: 50%; no access: up to 100%.</p><p>If an included area was missed, contact us within 24 hours and we’ll arrange a return to correct it.</p><p><a href="${process.env.APP_URL}/policies/cancellation">Cancellation policy</a> · <a href="${process.env.APP_URL}/policies/service">Service policy</a></p><p>Carolina Homekeeping Co.<br>YOUR HOME, HANDLED.</p></div>`;
