...
export function AppointmentPicker({slots,value,onChange}:{slots:AppointmentOption[];value:string;onChange:(id:string)=>void}) {
  ...
  return <>
    ...
    <div className="slot-list">{slots.filter(s=>localDay(s.starts_at)===activeDay).map(s=><button className={'slot-choice '+(value===s.id?'selected':'')} aria-pressed={value===s.id} type="button" onClick={()=>onChange(s.id)} key={s.id}><strong>{localTime(s.starts_at)} start</strong><span>Est. finish {localTime(s.ends_at)}</span></button>)}</div>
  </>;
}
